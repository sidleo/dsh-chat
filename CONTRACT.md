...
- **交付文件要真发出去**：hub 会把会话里的 `deliverables/presented`（agent 用 `present`
  ...
  **多个文件要尽量合成一条消息**，不要一条一个文件地刷屏——飞书用 `post`（富文本）消息：
  顶层 `files` 附件区可放多个 `file_key`（文件名/大小由服务端回填，客户端传 name 无效），
  图片用 `{"tag":"img","image_key":…}` 内嵌正文。发送前先本地校验（存在、是普通文件、非空、
  不超上限），失败要把**具体原因**回给用户并落 `lastError`——只把文件名写在回复文字里，
  用户是拿不到文件的。
