...
- **交付物要尽量少占消息、且不带文字**（真机要求）：
  飞书卡片组件里**没有文件/附件组件**，只有 `img`，所以 **图片直接内嵌进那张过程卡**
  （`{"tag":"img","img_key":…}`，不再单发消息；过程展示为 off/post 时退回附件消息）；
  **其余文件合成一条 `post` 消息**，只用顶层 `files` 附件区装 `file_key`（可多个，
  文件名/大小由服务端按文件元数据回填，客户端传 name 无效），正文留空——
  不要写"交付文件"之类的前缀，也不要逐行列文件名。
