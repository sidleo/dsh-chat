...
- **交付物合成一条不带文字的附件消息**（真机要求，图片同样算附件）：
  飞书用 `post`（富文本）消息，**正文留空**（`{"zh_cn":{"content":[]}}`），全部成品放进
  顶层 `files` 附件区（可多个 `file_key`；图片也按文件上传后进附件区，附件区只认
  `file_key`，不认 `image_key`）。文件名/大小由服务端按文件元数据回填，客户端传 name 无效。
