...
- **卡片要反应状态**：过程卡标题有 running / 等你确认 / ✅ 已完成 / ⚠️ 未正常完成 四态，配色同步
  ...
  是最容易被当成卡住的呈现问题。
- **过程信息要收进折叠面板**：工具调用与思考（reasoning 摘要）合并进**一个** `collapsible_panel`，
  默认收起、展开看全部；标题只写数量（`工具与思考(N)`），不写前缀也不写"最新一条"；工具行要带参数摘要
  （只有工具名等于没信息）。过程行数有上限（超出丢最旧的）。一轮可能有上百次工具调用，
  因此过程刷新按最小间隔合并（1.2s），收尾一定再刷一次——一次 patch 是整卡重写，不能每个事件都刷。
