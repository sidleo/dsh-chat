...
- **卡片要反应状态，但状态只写在卡片头那一条线上**：过程卡标题＝桌面的整轮控件文案——
  running `深度求索中，用时 X`（「用时」后**没有空格**，秒数不补零）、结束 `用时 X`（秒数补零）、
  失败 `处理失败`、被中断 `已停止`；配色同步（蓝 → 绿 → 橙）。**不再单独写
  「✅ 已完成 / ⚠️ 未正常完成」**（完成与否由颜色表达），标题也不带机器人名前缀
  （卡片本就在该机器人的会话里）。"处理完还写着正在处理"是最容易被当成卡住的呈现问题。
