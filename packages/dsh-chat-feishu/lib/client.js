...
var FEISHU_SOURCE_FIELDS = Object.freeze([
  "channel",
  "conversationType",
  "senderId",
  "chatId",
...
var LARK_SCOPE_KEYS = LARK_IDENTITY_SCOPES.map((item) => item.key);
var LARK_LAYER_LABELS = Object.freeze({
  global: "\u5168\u5C40",
  direct: "\u79C1\u804A",
  group: "\u7FA4\u804A"
});
var LARK_LAYER_KEYS = Object.keys(LARK_LAYER_LABELS);
...
function LarkIdentityEditor({ botId, value, conversations, chatUi, connection, translate, onChanged, scope = "direct" }) {
  ...
  const scopeRow = ({ key, label, id }) => {
    ...
    return h(
      ...
      }, LARK_SCOPE_CHOICES.filter((option) => !(key === "global" && option.value === "inherit")).map((option) => h("option", {
        key: option.value,
        value: option.value
      }, t(option.label))))
    );
  ...
  return h(
    ...
    targets.length === 0 ? h("p", { className: "dchat-cardDescription" }, t("\u8FD8\u6CA1\u6709\u6307\u5B9A\u8BBE\u7F6E")) : h("div", null, targets.map((target, index) => h(
      "div",
      {
    ...
    probe.phase === "ready" && info ? h("p", {
      ...
      info.larkCli ? info.larkCli.user?.onBehalfOf?.openId ? `${t("\u767B\u5F55\u4EBA")} ${info.larkCli.user.onBehalfOf.userName ?? info.larkCli.user.onBehalfOf.openId}` : `${t("\u7528\u6237\u8EAB\u4EFD")} ${identityText(info.larkCli.user, t("\u6CA1\u6709\u7528\u6237\u767B\u5F55"))}` : null
