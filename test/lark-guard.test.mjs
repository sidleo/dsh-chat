...
function createGuard({ mode = 'bot-only', locate, policyFor, channelId = 'feishu' } = {}) {
  return createLarkCliGuard({
    channelId,
    locate: locate ?? ((sessionId) => (sessionId === SESSION ? { channelId: 'feishu', botId: BOT_ID, key: 'p2p:ou_x' } : undefined)),
    policyFor: policyFor ?? (async (botId) => (botId === BOT_ID ? { mode, profileName: PROFILE } : null)),
    logger: silentLogger,
  });
}
...
test('规则本身：缺 profile / 缺身份 / bot-only 下用 user，都被拒且说清怎么改', () => {
  ...
  const userInBotOnly = evaluateLarkSegment({
    segment: `lark-cli --profile ${PROFILE} im +messages-send --as user --text 测试`,
    profileName: PROFILE,
    mode: 'bot-only',
  });
  assert.match(userInBotOnly, /只用应用身份/);
...
test('会改本机全局状态的写法一律拒绝（profile use / --use / --global / auth logout）', () => {
  for (const segment of [
    ...
    const reason = evaluateLarkSegment({ segment, profileName: PROFILE, mode: 'bot-only' });
...
test('门禁：bot-only 下即使补了 profile，--as user 仍然被拒；user-allowed 才放行', async () => {
  const botOnly = createGuard({ mode: 'bot-only' });
  const denied = await botOnly.evaluate(bashCall(`lark-cli --profile ${PROFILE} im +messages-send --as user --text x`));
  assert.equal(denied?.kind, 'deny');
  assert.match(denied.reason, /只用应用身份/);

  const allowed = createGuard({ mode: 'user-allowed' });
  assert.equal(
    await allowed.evaluate(bashCall(`lark-cli --profile ${PROFILE} im +messages-send --as user --text x`)),
    null,
  );
});
...
test('门禁：策略从运行期读（改完设置立刻生效，不用重启）', async () => {
  let mode = 'bot-only';
  const guard = createGuard({ policyFor: async () => ({ mode, profileName: PROFILE }) });
