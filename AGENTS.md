...
| P6 | 平台化：会话渠道标识、更新面板、i18n 完整化 | 会话渠道标识 ✅（工作区命名「渠道 · 机器人」、会话标题加「渠道 · 」前缀，均幂等）；版本与更新面板 ✅（设置页底部：内核/契约/各渠道包版本与状态、数据与日志目录、更新方式，`check` 会与 package.json 对账）；i18n 完整化待做（英文界面部分覆盖） |
