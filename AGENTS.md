...
packages/dsh-chat/             Hub：设置页入口 + 渠道注册表 + 共享内核（命令/策略/上下文增强/会话桥/设置）
  ...
  client/                      浏览器：设置页 section、共享 UI 组件与 hook
                               （bot-shared-settings.js = 工作区/Agent 预设/访问策略三块，
                                 ...
                                 setting-groups.js = 机器人设置页的分类导航与分组容器，
                                 scope-switcher.js = 分私聊/群聊的设置项共用的场合切换器，
                                 help-hint.js = 帮助图标（把"想知道再看"的说明收进问号），
...
- **机器人设置页是分组的，新加设置项必须放进分组里**：这一页是**逐轮追加**长出来的
  （10 张卡平铺、实测 3200+px 高，想改「任务过程展示」得盲滚两千像素），所以按职能分了四组：
  **运行环境**（工作区/Agent 预设/默认模型）、**权限与身份**（属主/访问策略/lark-cli 身份）、
  **呈现方式**（控制面板显示项/任务过程展示/卡片友好回答）、**能力**（上下文增强/主动投递）。
  分组实现在 hub 的 `client/setting-groups.js`（`SettingGroups`，经 `chatUi.components` 下发给渠道），
  **只做归类与跳转，不改任何子组件的实现与保存路径**；顶部一排分类是吸顶的，
  点一下滚到那一组，滚动时按 `IntersectionObserver` 反高亮，**用键盘/读屏走的是同一组标题**。
  ⚠️ **新加一项就把它放进 `BotCard` 的 `settingGroups` 数组里**——顺手写在末尾会让它
  **掉在最后一组外面**：页面照常渲染、卡片照常显示、单测全绿，只是没有标题、也没有分类能跳到它。
  布局守门专门钉了这条（"有 N 张设置卡片没被分进任何一组"），注入一张孤儿卡立刻红。
  头卡（机器人名称/状态/重连/移除接入）**不属于任何组**，它带 `data-card="bot-header"`，
  守门据此把它排除在"设置卡"之外（`Panel` 现在会透传这类多余 props）。
  微信页同一条口径（少了属主与 lark-cli 身份，组标题一致）——两个渠道的组名与顺序要**保持一致**，
  否则用户换渠道得重新认一遍页面。
- **分场合的设置项只有一套交互语言：页头「正在设置」场合切换器（全局 / 私聊 / 群聊）**。
  这一页原本有 4 个分叉项、**每一项各发明一种画法**——访问策略"并排两块"、任务过程展示
  "竖排两个下拉"、控制面板显示项"8行×2列勾选表格"、上下文增强"弹窗里两个页签"
  （lark-cli 身份还有四层）。用户的心智模型是「**一个设置项 × 几个场合**」，页面却要求他学四套。
  现在：**场合是页面级状态**（`BotCard` 的 `scope`，默认全局），页头选一次，
  下面所有分叉项都只画那一层。实现在 hub 的 `client/scope-switcher.js`（`ScopeSwitcher`）。
  ⚠️ **编辑器都收 `scope` prop，且"没给就照旧全画"**（向后兼容）：渠道页还没接切换器时不会少画东西；
  给了就只画那一个（`AccessPolicyEditor` / `PanelSectionsEditor` / `ScopedModeEditor` /
  `ContextEnhancementEditor`，后者连弹窗里的页签都不画了）。
  ⚠️ **每一个分叉项都只画当前层，一个例外都没有**（lark-cli 身份也一样）。
  它曾经是例外——画「全局 + 当前层」两层，理由是"选了继承要看得见上一层"——
  但**真机截图否掉了这个理由**：切到"全局"时卡里冒出一个"私聊"下拉、切到"私聊"时又冒出"全局"，
  用户以为主体切换器坏了（"全局的配置里有私聊的选项，私聊的配置里又有全局的选项"）。
  想看上一层就切到那一层（全页一致的行为）。另外**全局层不给「继承上一层」选项**：
  它上面没有层，选了会落回默认、看着像没生效。
  `LARK_SCOPE_KEYS`（两个**分类层**，供"指定设置"分类用）与 `LARK_LAYER_KEYS`（三层）
  不是一回事——用错会让 `scope === 'global'` 被误判成"认不出的层"而落回私聊（真栽过一次）。
  守门钉了三条：切换器恰好一个选中项、lark **只有当前层**那一个下拉、其余两层不出现。
- **分层：全局是底板，私聊/群聊默认继承、可单独覆盖**。四个设置项
  （`accessPolicy` / `stepPush` / `panelSections` / `contextEnhancement`）在盘上都是
  **`{ global, direct, group }`**，后两层为 **`null` = 继承全局**、有值 = 覆盖。
  通用机制在 `shared/scoped-config.mjs`（`migrateToLayered` / `resolveScope` / `writeScope` /
  `isInherited`），**运行期与设置页读值都必须走 `resolveScope`**——各写一份回落逻辑，
  早晚漂移成"设置页显示的是 A、运行时拦的是 B"（安全面上最难查的形态）。
  ⚠️ **迁移铁律：迁移只改变"以后怎么改"，不改变"现在是什么"**。老数据是 `{direct, group}`
  两份平级，按"尽量保留用户现有差异"迁：
  **两份相同 → 提成 global、两层都继承**（以后改全局两层都变）；
  **两份不同 → 取更保守那份当 global、两份原样保留为覆盖**（实际判定一个都不变）。
  两条迁错方向都很难查：把"私聊放开"迁丢 = 机器人突然不认人；
  把"群聊收紧"迁丢 = **机器人在群里裸奔**。`test/scoped-config.test.mjs` 逐个用例断言
  "迁移前后各层生效值一致"（9 条），改迁移逻辑先看它。
  ⚠️ **访问策略的"更保守"是有明确口径的**：`allowlist` 比 `open` 保守；
  同为 `allowlist` 名单更小更保守；同为 `open` 不许命令的更保守
  （`access-policy.mjs` 的 `moreConservative`，对齐 `evaluateAccess` 的放行条件）。
  ⚠️ **写值时必须把那一层标成"覆盖"**：`stepPush` 存在 `bot.stepPushInherit` 数组里，
  `setStepPush` 写值却忘了从 inherit 里去掉该层，读回来又会被全局盖掉——
  现象就是"设置页改了、群里没变"。运行期 patch（`patchRuntime`）也要**三层一起就地改**
  （只改值不改 inherit，桥仍按旧的继承关系取值）。`test/feishu.test.mjs` 两条三层测试钉住了。
  ⚠️ **在"继承中"的层上改任何一项 = 建立一份覆盖**（以**当前生效值**为底板，不是重写默认）：
  重置成默认会把继承来的名单/开关清掉。界面必须写清这一点（`dchat-layerNote` +
  「恢复继承全局」按钮），否则用户会遇到"我明明在这里关了、怎么又开了"。
  ⚠️ **`contextEnhancement` 的 `targets`（指定人/指定群）不参与继承**：它比场合更细，
  命中就用它、没命中才落到场合层；`merge:'append'` 叠的是**该场合生效的那份**。
  ⚠️ **最早的共用开关格式（`groupEnabled`/`directEnabled`）要单独判**：它两个开关是分开的，
  按 v2 老形态迁移会把"只开了一边"抹平——`normalizeContextConfig` 里必须**排在 v2 分支之前**判。
- **说明文字收进问号，常驻只留"影响判断"的那句**。设置页的说明文字总量远超控件——
  实测整页 **5000+ 字**，单是「任务过程展示」一张卡就 622 字；真正要改的下拉反而被挤到看不见的地方。
  口径（**别把该常驻的也藏起来**）：
  - **常驻**：影响"改了会不会生效"的一句（"只对新建会话生效"）、当前状态（"继承全局"）、
    失败原因——这些藏起来就是"失败不可见"；
  - **收进问号**：这是什么、为什么这么设计、权限怎么开、字段为什么可能缺。
  实现是 `client/help-hint.js`（`HelpHint`，经 `chatUi.components` 下发给渠道）：
  **圆形边框问号 + 悬浮气泡**（用户定的形态：不要"点开在页面里展开一段"——
  展开会顶动下面的内容、打断视线，气泡浮在上面看完就走）。
  ⚠️ **不用原生 `title`**：自定义气泡已经承担显示，两套会同时冒出来（原生那个还有 1 秒延迟、
  样式不受控）。可访问性靠 `aria-label` + `role="tooltip"`，并把完整文本放进
  `.dchat-visuallyHidden`（气泡对读屏不一定可达，内容不能因此丢）。
  ⚠️ **气泡的定位基准必须是「卡片头」（`.dchat-cardHeader`），不能是「说明那一段」。**
  这里真机翻过一次车（截图为证）：把基准挂在说明段落上时，「上下文增强」那张卡的描述
  **整句话都收进了气泡**、那一行就只剩一个问号，于是基准宽度 = 标题宽度（约 100px），
  气泡被压成**又窄又高的一条竖线**（1182px 宽的截图里气泡只有 ~100px）。
  卡片头永远占满卡片宽度，所以 `left:0; right:0` 一定有整卡可读。
  ⚠️ 教训是**守门只断言了"可见"**——可见但没法读，等于没做到。
  现在同时断言**宽度 ≥ 140px**（不足就红：`帮助气泡只有 115px 宽`），
  反向验证过（把基准改回说明段落，54 项红）。
  另外 `.dchat-cardHeading` 要给 `flex: 1 1 auto`（否则它按内容收缩，描述行也跟着变窄）。
  ⚠️ **写 CSS 时别在模板字符串里放反引号**：`styles.js` 整个是一段 `\`...\``，
  注释里写 `.类名` 会**提前结束字符串** → 整个 bundel 解析失败 → 页面白屏、
  而且守门的报错只是"页面没有产出测量结果"（很难从现象猜到原因）。这个坑踩过两次。
  ⚠️ 显示走 `:hover` **和** `:focus-within`：触屏没有 hover，点击 = 聚焦也要能看到。
  ⚠️ **问号必须挨着它解释的那句话**：曾经把它单独放进一个 div，渲染出来是**孤零零一个圆点**
  （真机反馈"这行只有个问号"）。
  守门钉三条：**默认不可见**（挡住"退回常驻展开"）、**聚焦后可见**（证明那条 CSS 规则真接上了）、
  气泡里有文字（挡住"说明被删了"）。去掉 `:focus-within` 或把 `display` 写成常驻，立刻红。
- **「像下拉但能手打」用 `.dchat-combo`（工作区目录）**。原来用原生 `<input list>` + `<datalist>`，
  那个下拉箭头由**浏览器/系统**画，边框、圆角、箭头都和页面上其它 `.dchat-select` 不一样
  （真机反馈"风格不一致"）。现在自己拼外壳：外观照抄 `.dchat-select` 的 token，箭头是自家按钮。
  ⚠️ 候选列表要**与外框的边框对齐**（`left/right: -1px`）：居中时两条边框错开一像素、
  在圆角处叠出一小块深色三角（截图放大能看到）。
- **文字总量是这次改动的硬指标**。基线（HEAD）整页可见文字 **1518 字**，现在 **1079**（-29%）。
  动手前先量一遍再改（用无头浏览器统计"真正可见"的文字：排除 `.dchat-helpTip` 与
  `.dchat-visuallyHidden`，并先 `blur()`——否则聚焦中的气泡会被算进去）。
  剩下的基本是**控件标签与数据**（候选列表、白名单行），不是说明性文案。
- **上下文增强是内嵌卡片，不是"入口 + 弹窗"**。它以前是页面上唯一的另类形态（点开才知道是什么），
  与其余 9 项的"直接铺出来"不一致。现在同形态，**保存仍是显式按钮**
  （与工作区同一条理由：这里有一段多行提示词，打到一半就提交会存进半句话；
  列表增删也一并走这个按钮，不会出现"删了一条却要另点一次保存"），
  另有「放弃改动」只在真改过时出现。
  ⚠️ **没有全局层的项要如实说**：页面场合是 `global` 而该项（如 `stepPush`）只有私聊/群聊时，
  只画**第一个作用域**并补一行"这一项目前只有私聊/群聊两份（没有全局层）"——
  画两个下拉等于把场合收敛回退了，什么都不说则用户以为切错了。
- **渠道页只画这个渠道真的支持的东西**（真机反馈："微信的配置中存在很多微信不支持的功能"）。
  查下来三处，都不是"少画了一点"，而是**整块摆设**：
  ① 微信的 `/menu` 发的是**文本**命令清单（`commands.mjs` 的 `reply` 那条路），**根本不画卡片**，
     所以整张「控制面板显示项」在微信上一个开关都不生效；
  ② 那张卡里的「渠道设置（任务过程展示等）」「渠道动作按钮（重连等）」，微信**没有这两个东西**
     （它不实现 `panel.fields` / `panel.actions`）；
  ③ 访问策略的帮助写着"属主在「权限与身份」那一组里单独设置"——**微信页上没有属主卡**
     （它的属主 = 扫码绑定的人，`ownerUserId`，不可改），把用户指向不存在的东西。
  修法：渠道用 `capabilities` 把事实声明出来（`panel: false` / `groups: false`），页面照声明画；
  微信页去掉整个「呈现方式」组（它只有那一张死卡），`SettingGroups` 会自动跳过空组。
  判断标准一句话：**这张卡/这个开关，在这个渠道上有没有一条真实的代码路径会读它？**
  ⚠️ **声明要与页面一致**，所以布局守门把它们对起来了：假微信渠道声明 `panel: false`，
  守门断言"声明没有面板的渠道，页面上不许出现「控制面板显示项」"，反之也红（两个方向都验过）。
  ⚠️ **hub 共享组件的文案不许引用页面结构**（"去某张卡里设置"）：`AccessPolicyEditor` 现在收
  `ownerHint`（渠道说明属主是谁）、`showInheritance`（单会话类型渠道传 false——
  微信只有私聊，"继承全局/恢复继承"会让用户去找一个页面上看不到也改不了的"全局"）。
  帮助文案里也不该出现"群里…"这类只对一个渠道成立的说法（已去掉两处）。
- **来源字段按渠道声明来（`capabilities.sourceFields`）**。实测**两个渠道都没实现**
  `senderName` / `conversationTitle`（schema 与限长表里都有，但没有任何一处填它），
  微信还多两条：`chatId` 恒等于 `senderId`（私聊）、`conversationType` 恒为 `direct`，
  `threadId` 根本没有——所以 8 个勾选框里微信只有 3 个有意义。
  **勾了没值 = 静默无效**，正是"配置里一堆用不上的东西"。
  现在渠道声明 `sourceFields`，编辑器只画那些；`CONTEXT_FIELDS`（schema/校验）**不动**——
  老配置里写了 `senderName` 的照样能读出来，只是不再提供新的勾选。
  守门把"声明的字段"与"画出来的 `[data-context-field]`"对起来（多画、少画都红，两个方向验过）。
- **hub 文案里不许出现只对一个渠道成立的说法**。"在群里 @ 一次机器人"这类指引，
  对 `groups: false` 的微信是**走不通的指引**（照着做永远等不到会话出现）——
  `DeliveryTargetsEditor` 现在收 `groups`，微信传 false，空态改成"先和机器人私聊一次"。
  同理去掉过：访问策略说明里的"群聊下任何成员 @ 它就行"、上下文增强里的"在哪个群说的"、
  「指定用户」的"只在该用户与机器人的私聊中生效"。
  判断标准：**这句话在另一个渠道上还成立吗？** 不成立就走 props 让渠道给。
- **单会话类型渠道不显示"场合"话术**（微信只有私聊）：访问策略的行标签「私聊」是纯冗余
  （整页管的就是那一种），`showInheritance: false` 时一并把标签也省掉；
  「继承全局 / 恢复继承」也不画——用户会去找一个**页面上看不到也改不了**的"全局"。
- **微信按"只给属主自己用"处理**（用户定的定位），所以它的设置页比飞书少三块：
  | 少了什么 | 为什么 |
  |---|---|
  | 「控制面板显示项」整组 | 微信 `/menu` 发文本、不画卡片（见 `capabilities.panel`） |
  | 「权限与身份」整组（访问策略） | 属主 = 扫码绑定的人，缺省策略（仅名单内 + 空名单）**恰好就是"只有属主能用"** = 微信常态；想加人得填 `from_user_id`（**微信不显示这个 id**，页面也没有选择器），想放开只能选「任何人可用」——在"给我自己用"的渠道上那是挖坑 |
  | 上下文增强里的「指定用户」 | 只有一个人在聊，再给他单开一份设置是多余的 |
  ⚠️ **去掉的是界面，不是能力**：`/allow` / `/deny` 仍可用（属主在聊天里发命令，`isOwner` 为真）。
  布局守门钉住了后两条（微信卡里出现「访问策略」/「指定用户」就红）。
  判断依据：**属性来自 `ownerUserId`（扫码那个人），而"谁能用"由 accessPolicy 决定**——
  两者都读同一份数据，界面只是不再提供一个"填不进正确值"的入口。
- **删代码要连"没人用了"的引用一起删**：微信页去掉访问策略后，`AccessPolicyEditor` 的解构
  与 `shared.accessPolicy` 就没人用了——留着会让下一个人以为它还在被用。
- **字典去重时正则要对准真实写法**：`i18n.js` 是 `export const zh = {`，
  而我的去重脚本写的是 `^const (zh|en) = \{`——**一个都没匹配上，"去重"静默什么也没做**，
  直到我去看构建警告才发现（与前面 `groups` prop 那次是同一类错）。现在脚本结束会打印
  各块键数 + 值冲突（它顺便查出一个真冲突：`收起` 在 en 块里同时有 `Collapse` 和 `Hide`）。
