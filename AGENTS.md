...
- **过程卡那两行就是桌面端的两层折叠**：卡片头＝桌面的整轮控件，**只在运行中**写
  `深度求索中，用时 X`；**结束之后一个字都不写**（完成/失败/中断都不写状态词、**也不显示已用时**），
  完成与否**只看颜色**（绿＝正常结束、橙＝失败或中断），失败原因在正文里
  （`本轮运行失败（<reason>）。`）。⚠️ Card 2.0 的 `header.title` 是**必填**，而**空串会让飞书连整条配色头
  一起不画**（真机反馈「颜色没了」）——所以「不写字」要传一个**零宽空格**
  （`INVISIBLE_TITLE`，U+200B；别用 U+FEFF，`trim()` 会吃掉它）。
...
- **「不显示过程」也要用卡片发答案**（真机要求"只发送最终答案也需要卡片"）：关掉过程**不等于**
  ...
  （`renderAnswerCard`：同一套配色（结束＝绿、失败＝橙；标题传零宽字符、只有颜色），
  正文只有答案 markdown，
