...
- **「渠道设置」入口按渠道决定**：左栏那个入口由渠道注册的 `capabilities.setup.label` 控制——
  给了才显示（微信「扫码接入」，那是唯一的接入入口），不给就不显示（飞书没有渠道级表单：
  点进去只有一行 dataDir，跟机器人设置页几乎一样，用户问过"渠道设置是做什么用的、是不是有点多余"）。
  空列表的接入说明同样由渠道给（`setup.hint`：飞书"把凭据加进 config.json"、微信"扫码"），
  没给才退回 hub 的中性句。两项**各自独立**，只给一半是合法形态；写兜底那句时注意 `t` 的声明
  要排在使用之前（声明在使用之后 = 走到这条分支就 TDZ 崩溃）。布局守门让两个假渠道各覆盖一半，
  并断言"渠道没给 hint 时空态用的是中性句"——只测"都有 hint"的形态是测不出崩溃的。
