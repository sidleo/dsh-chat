...
- **新建机器人接入（飞书）**：`bot.add` 的顺序是刻意的——**先验凭据，再写任何东西**：
...
- **访问策略白名单只显示 id**（真机反馈"不方便管理"）：白名单里存的只有平台 id，
  ...
  换名字要靠 `contact` / `im:chat:readonly` 权限；开通后点「重新连接」立刻重取。
...
| P7 | 机器人自助接入与可用性 | ✅ 飞书「新建机器人接入」：在设置页填自建应用的 App ID + App Secret（可选属主）即可加机器人——**先验凭据再写任何东西**（探针换 `tenant_access_token`），`botId`/`secretRef` 由 `sha256(appId)` 推导（重复接入直接拒绝），落盘失败回滚刚写的凭据；属主留空时客户端紧接着调 hub 的 `bot.access-policy.open-scope` 把私聊放宽到「任何人可用」（新机器人没有属主候选，默认策略谁都进不来），那一步失败只告警、绝不静默。另有：访问策略白名单显示「名字 + id」（飞书 `names.resolve`，查不到只显示 id + 原因）|
