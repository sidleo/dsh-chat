...
| P5 | 富媒体（图片/文件）与主动投递 | 主动投递文本+出站文件/图片（飞书）、飞书入站图片+入站文件已通；人在环回传已通：工具/思考收进折叠面板（默认收起、标题显示最新）、提问**内嵌在「正在处理」那张卡里**（Card 2.0，一页一题：单选按钮+输入框 / 多选勾选器 / 文本输入框，答完收起，无进度卡时退回独立卡片）；未做：微信收图、微信出站文件 |
