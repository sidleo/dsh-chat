...
| P3 | 微信渠道：移植 iLink 协议客户端、扫码登录、私聊收发 | ✅ 代码与测试完成（待重启上线验证） |
| P4 | 机器人命令（/help /new /status /version /stop /session /models /model /reasoning(s) /presets /preset）+ 访问策略（白名单 + 命令权限） | ✅ 代码与测试完成；菜单卡片、批量输入、压缩待做 |
| P5–P6 | 富媒体与主动投递、平台化（会话标识、更新面板、i18n…） | 待开始 |
